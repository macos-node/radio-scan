...
Usage:
    ...
    python3 duck_playlist.py --data-dir ~/RadioTuner   # append to a pre-vendoring log

DATA LAYOUT (mirrors radioscan.py)
    <data_dir>/duck/duck_log.jsonl      raw log, one line per track (source of truth)
    <data_dir>/duck/duck_log.csv        same data, spreadsheet-friendly
    <data_dir>/duck/duck_log.clean.*    --clean output
...
SCHEDULING
    ...
    Wed 09:00); the macOS original ran under launchd com.tigger.duckradio.
"""
...
def set_data_dir(base: str) -> None:
    """Point the module's four output paths at <base>/duck/, creating it."""
    global CSV_OUT, JSONL_OUT, CSV_CLEAN, JSONL_CLEAN
    d = Path(base).expanduser() / SOURCE
    d.mkdir(parents=True, exist_ok=True)
    CSV_OUT     = d / "duck_log.csv"
    JSONL_OUT   = d / "duck_log.jsonl"
    CSV_CLEAN   = d / "duck_log.clean.csv"
    JSONL_CLEAN = d / "duck_log.clean.jsonl"
